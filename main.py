...
def start_recording():
    ...
    out = cv2.VideoWriter(filePath + 'Video_' + file_name + '.avi', cv2.VideoWriter_fourcc('M', 'J', 'P', 'G'), 20,
...
while (True):
    ...
    if k == 114:
        if not isRecord:
            ...
            stop_recording()
            size_record()
            db_record.insert_record(numberCanal, "user", number - 1,
                                    str(filePath + 'Video_' + str(number - 1) + '.avi'), date_start, date_end,
                                    size_record(), 'avi', save_frame(image))
            isRecord = False
